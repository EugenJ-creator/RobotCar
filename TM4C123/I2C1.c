...
// Provide a function that initializes, sends, and receives the I2C1 module
...
/* This example accompanies the book
 ...
 Modified for RobotCar Project by Evgheni Jaruc
 */
...
void I2C1_Init(void){
  SYSCTL_RCGCI2C_R |= 0x0002;           // activate I2C1 Enable bit 1
  SYSCTL_RCGCGPIO_R |= 0x0001;          // activate port A  Enable bit 0
  while((SYSCTL_PRGPIO_R&0x0001) == 0){};// ready?    bit 1 for port A

  GPIO_PORTA_AFSEL_R |= 0xC0;           // 3) enable alt funct on PA6,7
  GPIO_PORTA_ODR_R |= 0x80;             // 4) enable open drain on SDA PA7    only
  GPIO_PORTA_DEN_R |= 0xC0;             // 5) enable digital I/O on PA6,7
                                        // 6) configure PB2,3 as I2C
  GPIO_PORTA_PCTL_R = (GPIO_PORTA_PCTL_R&0x00FFFFFF)+0x33000000;
  GPIO_PORTA_AMSEL_R &= ~0xC0;          // 7) disable analog functionality on PB2,3
  I2C1_MCR_R = I2C_MCR_MFE;      // 9) master function enable
  I2C1_MTPR_R = 24;              // 8) configure for 100 kbps clock
...
uint8_t I2C1_RecvByteStart(int8_t slave){
  ...
  do{
    ...
    data = (I2C1_MDR_R&0xFF);       // MSB data sent first
  ...
  return data;                  // usually returns 0xFFFF on error
}
  ...
  do{
    I2C1_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C1_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data = (I2C1_MDR_R&0xFF);       // LSB data sent last
    retryCounter = retryCounter + 1;        // increment retry counter
  }                                         // repeat if error
  while(((I2C1_MCS_R&(I2C_MCS_ADRACK|I2C_MCS_ERROR)) != 0) && (retryCounter <= MAXRETRIES));
  return data;                  // usually returns 0xFFFF on error
    ...
    data2 = (I2C1_MDR_R&0xFF);       // LSB data sent last
		...
		I2C1_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C1_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data3 = (I2C1_MDR_R&0xFF);       // LSB data sent last
		...
		I2C1_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C1_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data4 = (I2C1_MDR_R&0xFF);       // LSB data sent last
    retryCounter = retryCounter + 1;        // increment retry counter
  }                                         // repeat if error
  while(((I2C1_MCS_R&(I2C_MCS_ADRACK|I2C_MCS_ERROR)) != 0) && (retryCounter <= MAXRETRIES));
...
void I2C1_Recv6(int8_t slave, uint8_t data[]){
  ...
  do{
    ...
    I2C1_MCS_R = (0
                         ...
                         | I2C_MCS_RUN);    // master enable
    while(I2C1_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data[0] = (I2C1_MDR_R&0xFF);       // MSB data sent first
		...
		I2C1_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C1_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data[1] = (I2C1_MDR_R&0xFF);       // LSB data sent last
		...
		I2C1_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C1_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data[2] = (I2C1_MDR_R&0xFF);       // LSB data sent last
		...
		I2C1_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C1_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data[3] = (I2C1_MDR_R&0xFF);       // LSB data sent last
		...
		I2C1_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C1_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data[4] = (I2C1_MDR_R&0xFF);       // LSB data sent last
		...
		I2C1_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C1_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data[5] = (I2C1_MDR_R&0xFF);       // LSB data sent last
    retryCounter = retryCounter + 1;        // increment retry counter
  }                                         // repeat if error
  while(((I2C1_MCS_R&(I2C_MCS_ADRACK|I2C_MCS_ERROR)) != 0) && (retryCounter <= MAXRETRIES));
