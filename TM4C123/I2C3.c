...
// Provide a function that initializes, sends, and receives the I2C3 module
...
/* This example accompanies the book
 ...
 Modiefied for RootCar Project by Evgheni Jaruc
 */
...
void I2C3_Init(void){
  SYSCTL_RCGCI2C_R |= 0x0008;           // activate I2C3 Enable bit 3
  SYSCTL_RCGCGPIO_R |= 0x0008;          // activate port D  Enable bit 3
  while((SYSCTL_PRGPIO_R&0x0008) == 0){};// ready?

  GPIO_PORTD_AFSEL_R |= 0x03;           // 3) enable alt funct on PD0,1
  GPIO_PORTD_ODR_R |= 0x02;             // 4) enable open drain on SDA PD1    only
...
uint8_t I2C3_Recv(int8_t slave){
  int retryCounter = 1;
  do{
...
// Note for TMP102 thermometer only:
// Used to read the contents of the pointer register
uint32_t I2C3_Recv4(int8_t slave){
    ...
    data2 = (I2C3_MDR_R&0xFF);       // LSB data sent last
		...
		I2C3_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data3 = (I2C3_MDR_R&0xFF);       // LSB data sent last
		...
		I2C3_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data4 = (I2C3_MDR_R&0xFF);       // LSB data sent last
    retryCounter = retryCounter + 1;        // increment retry counter
  }                                         // repeat if error
  while(((I2C3_MCS_R&(I2C_MCS_ADRACK|I2C_MCS_ERROR)) != 0) && (retryCounter <= MAXRETRIES));
  return (data1<<24)+(data2<<16)+(data3<<8)+(data4);                  // usually returns 0xFFFF on error
}
...
// Note for TMP102 thermometer only:
// Used to read the contents of the pointer register
void I2C3_Recv6(int8_t slave, uint8_t data[]){
  ...
  do{
    ...
    I2C3_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_STOP    // no stop
                         ...
                         | I2C_MCS_RUN);    // master enable
    while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data[0] = (I2C3_MDR_R&0xFF);       // MSB data sent first
		...
		I2C3_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data[1] = (I2C3_MDR_R&0xFF);       // LSB data sent last
		...
		I2C3_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data[2] = (I2C3_MDR_R&0xFF);       // LSB data sent last
		...
		I2C3_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data[3] = (I2C3_MDR_R&0xFF);       // LSB data sent last
		...
		I2C3_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data[4] = (I2C3_MDR_R&0xFF);       // LSB data sent last
		...
		I2C3_MCS_R = (0
                       ...
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for transmission done
    data[5] = (I2C3_MDR_R&0xFF);       // LSB data sent last
    retryCounter = retryCounter + 1;        // increment retry counter
  }                                         // repeat if error
  while(((I2C3_MCS_R&(I2C_MCS_ADRACK|I2C_MCS_ERROR)) != 0) && (retryCounter <= MAXRETRIES));
  
		//(data1<<24)+(data2<<16)+(data3<<8)+(data4);                  // usually returns 0xFFFF on error
}
...
uint32_t I2C3_Send1(int8_t slave, uint8_t data1){
  while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for I2C ready
  I2C3_MSA_R = (slave<<1)&0xFE;    // MSA[7:1] is slave address
...
//  This will work but is probably not what you want to do.
// Returns 0 if successful, nonzero if error
uint32_t I2C3_Send2(uint8_t slave, uint8_t data1, uint8_t data2){
  while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for I2C ready
  I2C3_MSA_R = (slave<<1)&0xFE;    // MSA[7:1] is slave address
  I2C3_MSA_R &= ~0x01;             // MSA[0] is 0 for send
  I2C3_MDR_R = data1&0xFF;         // prepare first byte
...
// Returns 0 if successful, nonzero if error
uint32_t I2C3_Send3(int8_t slave, uint8_t data1, uint8_t data2, uint8_t data3){
  while(I2C3_MCS_R&I2C_MCS_BUSY){};// wait for I2C ready
  I2C3_MSA_R = (slave<<1)&0xFE;    // MSA[7:1] is slave address
  I2C3_MSA_R &= ~0x01;             // MSA[0] is 0 for send
  I2C3_MDR_R = data1&0xFF;         // prepare first byte
